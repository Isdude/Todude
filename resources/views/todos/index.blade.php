...
<a href="{{ route('todos.edit', ['user' => Auth::user()->id]) }}" class="user-info">
    <div class="user-details">
        ...
        <a href="{{ route('todos.index') }}" class="notification">
            <button aria-label="Completed" role="listitem">
                ...
                Inbox
            </button><span class="badge">{{ $todos->count() }}</span>
        </a>
    ...
    <ul class="todo-list">
        @forelse ($todos as $todo)
            ...
            <form action="{{ route('todos.status', $todo->id) }}" method="POST">
                @csrf
                @method('PUT')
                <input type="checkbox" name="completed" value="1" {{ $todo->status ? 'checked' : '' }} onchange="this.form.submit()">
            </form>

            <form action="{{ route('todos.destroy', $todo->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="button-dlt">🗑 Delete</button>
            </form>
        </div>

            </li>
            @empty
            <li class="empty">Add Your New To Do List</li>
        @endforelse
    </ul>
    </div>
</body>
